 ...
 * 微信公众号JOB
 */
public class WeChatServiceJob extends Job { 
	...
	public static void main(String[] args) throws Exception {
		...
		new WeChatServiceJob().execute(params);
	}
		...
		while (true) {
			try {
				...
				if (ZbusService.mqLength(producer) > 0) {
					...
					List<OpsMonitorWechat> list = null;

					if (preHourly != time.getHour()) {
						logger.info("开始获取全量种子");
						preHourly = time.getHour();
						list = OpsMonitorWechat.dao.find(String.format(" select * from %s ", OpsMonitorWechat.TableName));
					}

					if (null != list) {
						logger.info("种子数量:" + list.size());
						for (OpsMonitorWechat stock : list) {
