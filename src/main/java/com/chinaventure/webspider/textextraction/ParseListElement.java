...
//		return BLANK_REGEXP.matcher(node.text()).matches();
	...
	private static final Pattern TINY_REGEXP = Pattern.compile("comment|page",
	...
	private static final Pattern MAJOR_REGEXP = Pattern.compile(
			"article|entry|post|body|column|main|content|detail|text|list",
			Pattern.CASE_INSENSITIVE);
	...
	private static final Pattern MINOR_REGEXP = Pattern.compile("comment|combx|disqus|foot|header|menu|rss|shoutbox|sidebar|sponsor",Pattern.CASE_INSENSITIVE);
	private static boolean checkMinorAttr(String attr) {
		return MINOR_REGEXP.matcher(attr).matches();
	}
	
	public ParseListElement(Element element,int position) {
		this.element = element;
		this.position = position;
...
//		System.out.println("conten:"+ calcContentWeight());
	...
	private float calcStructWeight() {
		...
		for (int n = 0, r = this.texts.size(); n < r; n++) {
			...
			for (Node u = i.parent(); null != u && u != this.element; u = u.parent())
				o -= 0.11;//这个值原来是.1，但由于一些网页图文混排太严重
	...
	private float calcContentWeight(){
        ...
        for (Element t = this.element;null != t; t = t.parent()) {
        	...
        	/*
        	if(t.tagName().equalsIgnoreCase("ul")){
                e += .4;
            }
            
        	if(t.tagName().equalsIgnoreCase("li")){
                e += .4;
            }*/
